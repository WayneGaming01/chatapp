      ...
      async (err, decodedToken) => {
        ...
        Userdetails.findOne(
          ...
          async (err, doc) => {
            ...
            } else if (serverfindOne.server_owner === userforDetails.username) {
              return res.json({
                errors: {
                  invite_link:
...
exports.server_internal = async (req, res, next) => {
  const server_id = req.params.id;

  Server.findById(server_id, async (err, doc) => {
    if (err) return res.send(err);

    res.send(
      `You're viewing server id: ${server_id}<br><br>Server query: ${doc}`
    );
  });
};
